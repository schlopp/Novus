...
class BufferedDecoder(threading.Thread):
    ...
    def __init__(self, ssrc, output_func, *, buffer=200):
        ...
        if buffer < 40:  # technically 20 works but then FEC is useless
            raise ValueError(
                "buffer size of %s is invalid; cannot be lower than 40" % buffer
            )
    ...
    def _push(self, item):
        if not isinstance(item, (RTPPacket, SilencePacket)):
            raise TypeError(
                f"item should be an RTPPacket, not {item.__class__.__name__}"
            )
        ...
        if bufsize >= self.buffer_size * self._overflow_mult:
            ...
            self._overflow_mult = max(
                self._overflow_base, self._overflow_mult - self._overflow_incr
            )
            ...
            try:
                if not self._finalizing:
                    self._buffer.append(
                        SilencePacket(
                            self.ssrc,
                            self._buffer[-1].timestamp + Decoder.SAMPLES_PER_FRAME,
                        )
                    )
    ...
    def _initial_fill(self):
        ...
        if len(self._buffer) > 3:
            ...
            with self._lock:
                diffs = [
                    self._buffer[i + 1].sequence - self._buffer[i].sequence
        ...
        with self._lock:
            ...
            for x in range(1, 1 + self.buffer_size - len(self._buffer)):
                self._push(
                    SilencePacket(self.ssrc, start_ts + x * Decoder.SAMPLES_PER_FRAME)
                )
    ...
    def _packet_gen(self):
        while True:
            ...
            self._last_ts = getattr(
                packet, "timestamp", self._last_ts + Decoder.SAMPLES_PER_FRAME
            )
...
class BufferedDecoder2(threading.Thread):
    ...
    def _get_decoder(self, ssrc):
        ...
        if (
            not dec and self.reader.client._get_ssrc_mapping(ssrc=ssrc)[1]
        ):  # and get_user(ssrc)?
    ...
    def _initial_fill(self):
        ...
        try:
            ...
            if len(buff) > 3:
                ...
                with self._lock:
                    diffs = [
                        buff[i + 1].sequence - buff[i].sequence
